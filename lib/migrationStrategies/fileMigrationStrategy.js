 ...
 * @api private
 */
function positionOfMigration(migrationSet) {
    var numberMigrations = migrationSet.migrations.length;
    var migrationPosition = -1;

    for (var migrationIndex = 0; migrationIndex < numberMigrations; migrationIndex++) {

        if (migrationSet.migrations[migrationIndex].title == migrationSet.startingMigration) {
...
function migrationPosition(migrationSet) {
    ...
    if (!migrationSet.startingMigration) {
        ...
        migrationPos = positionOfMigration(migrationSet);
    }

    if(migrationPos == -1) {
        logger.error("Could not find migration: " + migrationSet.startingMigration);
        process.exit(1);
    }
